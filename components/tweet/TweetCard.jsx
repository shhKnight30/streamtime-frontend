...
export function TweetCard({ tweet }) {
  ...
  return (
    ...
    <div className="flex gap-4 rounded-xl border border-[var(--border)] bg-[var(--surface)] p-4 sm:p-5">
      ...
      <Link href={`/channel/${tweet.owner?._id}`} className="shrink-0">

        <div className="h-10 w-10 overflow-hidden rounded-full bg-[var(--surface-raised)]">

          <img

            src={tweet.owner?.avatar || "https://api.dicebear.com/7.x/avataaars/svg?seed=fallback"}
      ...
      <div className="flex flex-1 flex-col gap-2">
        ...
        <div className="flex items-start justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link href={`/channel/${tweet.owner?._id}`}>
            ...
            <span className="text-sm text-[var(--text-muted)]">@{tweet.owner?.username}</span>
